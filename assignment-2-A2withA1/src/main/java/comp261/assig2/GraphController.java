...
public class GraphController {
    ...
    @FXML
    private CheckBox distance_ch; //added 
    ...
    public void handleGoalAction(ActionEvent event) {
        ...
        // perform A* search and get the path edges
        pathEdges = AStar.findShortestPathEdges(graph, startLocation, goalLocation);
        //System.out.println("startLocation "+startLocation+" : "+"goalLocation "+goalLocation);
    ...
    public void handleShowConnectedComponents(ActionEvent event) { //called at components button
        ...
        tripText.setText(components+" connected components");
        drawGraph(graph);
    }

    // handleAddWalking calls the code to add Walking
    public void handleAddWalking(ActionEvent event) {
        System.out.println("Add walking event " + walking_ch.isSelected()); //reached 
    ...
    //added for challenge 
    public void toggleDistTime(ActionEvent event){
        if(distance_ch.isSelected()){
            Edge.setDistanceCost();
        } else {
            Edge.setTimeCost();
        }
    }
    ...
    public void drawPathEdges(ArrayList <Edge> pathEdges, GraphicsContext gc) {
        ...
        for (Edge edge : pathEdges) {
            ...
            path = edge.getFromStop().getId() +":"+ edge.getToStop().getId() +"    "+ edge.getTripId() +"     cost "+ edge.getTime() +"     :to goal "+ totalTime  +"\n" + path;
        }
        // TODO: add total time to the path
        
        tripText.setText("Start: " + startLocation.getName() + "\n" + path + "\nTotal time for this path: "+totalTime+" seconds");
    }
